...
**Built:** A SEPARATE Vedic (Parashari) engine beside the KP engine, forked downstream of ayanamsa. `backend/app/engines/vedic_engine.py` — `compute_vedic_chart(datetime_local, gmt_offset, lat, lon, target_date)` returns settings + lagna + 9 planets (sidereal sign/degree/nakshatra/pada, whole-sign house, D-9/D-10 varga sign, dignity, aspected houses) + whole-sign house occupancy + current dasha path. `backend/app/engines/vedic_dasha.py` — Vimshottari seeded from the Moon, **current-path only** (no full tree), four levels MD→AD→PD→SD via true-tropical-solar-year Sun transits. Settings block `{ayanamsa: lahiri, houses: whole_sign, nodes: mean}` on every result. **KP engine untouched** — `git diff --name-only` on tracked files is empty; only 4 new files added.
**Shared vs forked:** shares ONLY the tropical layer + the time→Julian-Day helper (`ephemeris_engine._julian_day_ut`, imported). Lahiri (`swe.SIDM_LAHIRI`), whole-sign (`swe.houses_ex(...,b'W',...)`), MEAN node (`swe.MEAN_NODE`, Ketu=Rahu+180) are all in the fork. Node choice was NOT hardcoded in shared code (it lives in the KP engine's downstream `_compute_planets`), so nothing was parameterized in shared code and the KP path was not flipped.
**Contamination check (grep):** no KP constant is USED in the new module — `grep -E "swe\.TRUE_NODE|swe\.SIDM_KRISHNAMURTI|b'P'|b'B'"` over the vedic files = NONE. The only textual matches for "Krishnamurti"/"TRUE_NODE" are comments documenting the fork boundary ("never TRUE_NODE"). Vedic constants used: `swe.SIDM_LAHIRI`, `swe.MEAN_NODE`, `b'W'`.
**Files changed:** `backend/app/engines/vedic_engine.py` (new), `backend/app/engines/vedic_dasha.py` (new), `backend/tests/test_vedic_engine.py` (new), `backend/scripts/vedic_validate.py` (new), `HANDOFF.md` (this entry). No existing tracked file modified.
**Tests run:** `SE_EPHE_PATH=$(pwd)/ephe python -m pytest -q` (backend) → **29 passed, 5 skipped**. The 5 skips are the field-A JHora tests, which SKIP with "BLOCKED ON FIXTURES". 17 structural self-tests pass (Ketu=Rahu+180, houses tile 12, classical D-9/D-10 mapping locked, dignity/aspect structure, 4-level dasha nesting, settings block). `python scripts/vedic_validate.py` prints the field-A BLOCKED banner + a field-B smoke table.
**Known issues / deferred — BLOCKED ON FIXTURES:** `backend/tests/fixtures/vedic_fixtures.json` does not exist, so per spec the functions are built and validation STOPS. Field A (placements/houses/vargas/dasha vs JHora) and the field-B 5-chart review table both need the fixture birth data (charts vedic_01.._05) and are unrun; **no JHora value was fabricated**. Field B (dignity + graha drishti) is COMPUTED, not JHora-validated — pending a manual textbook spot-check. Two conventions inherited-and-flagged for fixture reconfirmation: (a) `FLG_TRUEPOS` (matches the KP engine's JHora-true-position finding; a per-planet, not constant, effect); (b) true-tropical-solar-year dasha length (same technique as `dasha_engine.py`; 365.25 vs true-solar shifts SD by minutes). Moon moolatrikona boundary set to Taurus 4°; "great friend/great enemy" are compound (Panchadha) categories the spec defers, so never emitted in v1. `scripts/check_allowed_files.py` absent → `git status --short` stands in. Not merged (PR to main only, per spec).
**Next agent should read:** `backend/scripts/vedic_validate.py` (expected `vedic_fixtures.json` schema is documented in its header), `backend/app/engines/vedic_engine.py`, `backend/app/engines/vedic_dasha.py`.
**Tempted but did not:** modify/refactor/convert the KP engine or `ephemeris_engine`; parameterize the KP node choice; build divisionals beyond D-9/D-10, special lagnas, a dasha level below Sookshma, the full nested dasha tree, or any yoga; fabricate fixture values; claim dignity/aspects are "tested"; merge the PR.
**Question (only if BLOCKED):** Provide `backend/tests/fixtures/vedic_fixtures.json` with the 5 JHora-golden charts (schema in `scripts/vedic_validate.py`) so field A and the field-B 5-chart review can run. Which Lahiri variant and which dasha year-length + `FLG_TRUEPOS`/apparent setting did JHora use for these exports?

## frontend-wiring — agent/claude/frontend-wiring — 2026-06-27 16:15 — Claude Code
**Built:** Fixed four frontend wiring defects (backend untouched). (1) Chart page now displays the **ascendant sign** (+ degree), a **Sign** column on the planetary positions table, and a **Sign** column on the house cusps table — previously ascendant and planet/house signs were not surfaced. The response is wrapped as `{chart_id, chart, ...}` (confirmed against `backend/app/routers/chart.py` `ChartGenerateResponse`), so `chartData.chart.*` access was already correct; the fixture `chart.sample.json` is the *inner* `chart`. (2) After a successful generate, the inner chart is persisted to `localStorage["junopath_chart"]` (`JSON.stringify(data.chart)`); the three Career/Finance/Relationship buttons were already `<Link>`s to `/predict/{career,finance,relationship}` and now have data to consume. (3) The three predict pages, in **non-fixture mode**, now read `localStorage["junopath_chart"]`, show "Please generate your chart first" with a link to `/chart` when empty, get the JWT via `supabase.auth.getSession()`, and `POST {chart}` to `` `${process.env.NEXT_PUBLIC_API_URL}/predict/{domain}` `` (the hardcoded Azure URL was removed; backend `PredictionRequest` requires a `{chart}` body, which the old code never sent). (4) Login page gained a "Sign in with password instead" toggle that reveals email+password fields, calls `supabase.auth.signInWithPassword({email, password})`, and redirects to `/chart` via `useRouter().push`; the magic-link form (and its rate limit) is still the default with a "Use magic link instead" toggle back.
**Files changed:** `frontend/app/chart/page.tsx`, `frontend/app/predict/career/page.tsx`, `frontend/app/predict/finance/page.tsx`, `frontend/app/predict/relationship/page.tsx`, `frontend/app/login/page.tsx`, `HANDOFF.md` (this entry). No backend, test, engine, fixture, or `globals.css` files touched.
**Tests run:** `npm install` then `npm run build` in `frontend/` → **Compiled successfully**, 12/12 static pages generated, zero errors (`/chart`, `/login`, `/predict/{career,finance,relationship}` all built). `node_modules` was absent in this worktree so a fresh `npm install` (375 packages) was required first.
**Known issues / deferred:** Could **not** verify the live Vercel value of `NEXT_PUBLIC_API_URL` from this environment (no Vercel access). `.env.example` documents the key; per the task it must be set in Vercel to `https://askjunopath-backend.kindtree-c857c99f.centralindia.azurecontainerapps.io`. Code falls back to `http://localhost:8000` only when the var is unset. localStorage shape stored is the **inner chart** (`data.chart`), matching what the predict engines/`PredictionRequest.chart` consume and what `chart/dasha/page.tsx` already expects. Predict pages keep the existing `NEXT_PUBLIC_USE_FIXTURE === "true"` fixture path unchanged.
**Next agent should read:** `frontend/app/chart/page.tsx` (localStorage write + display), `frontend/app/predict/*/page.tsx` (localStorage read + POST), `backend/app/routers/predict.py` (`PredictionRequest` body contract).
**Tempted but did not:** touch any backend/test/engine/fixture file; change the chart response shape (it is correctly wrapped); alter `globals.css` or hardcode any color; modify the fixture-mode branch in the predict pages.
